...
import { useSelector, useDispatch } from "react-redux"; // Importes do Redux
...
import { limparRascunho, salvarProgresso } from "@/app/redux/slices/draftSlice";
...
const draft = useSelector((state: RootState) => state.draft.draft)    // Calculando se tem rascunho de forma dinâmica (como era o temRascunho)
    const temRascunho = !!draft && Object.keys(draft).length > 0;

    // Estado local da tela do formulário
    const [clientes, setClientes] = useState<Cliente>(
        clienteExistente || new Cliente(null, '', '', '', "ATIVO")
    );

    // 2. Preenche o formulário se houver rascunho na inicialização
    useEffect(() => {
    ...
    }, [temRascunho, draft, clientes.nome, clientes.cpf]);

    // 3. Atualiza o estado local e dispara a action para salvar no Redux + Cookie
    ...
    // 4. Salva no banco de dados e limpa o rascunho via Redux
    const handleSalvar = async (formData: FormData) => {
        ...
        // Limpa o estado global e o cookie
        dispatch(limparRascunho());
